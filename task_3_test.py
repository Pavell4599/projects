...
#'идеальное здоровье', 'нормальное здоровье', 'допустимое здоровье', 'предболезненнное состояние', 'болеет'
class Puppy:
...
class Dog:
    ...
    def heal_all(self):
        for i in range(len(self.puppies)):
            if self.puppies['dog_' + str(i)].state != 0:
                self.puppies['dog_' + str(i)].get_treatment()
...
dog_1 = Dog(7)
dog_2 = Dog(3)
veter = Vet('Алексей Ветеренар', dog_1)
...
veter = Vet('Алексей Ветеренар', dog_2)
